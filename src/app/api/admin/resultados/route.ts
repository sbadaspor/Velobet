...
/** Apaga por completo os resultados guardados de uma etapa (Classificação
 * Geral, Sprint, Montanha, Juventude). A etapa volta a ficar "Pendente" —
 * só reaparece se voltares a importar/colar os resultados. Usado pelo
 * botão "Apagar Resultados" no admin (com dupla validação no cliente). */
export async function DELETE(req: Request) {
  const admin = await requireAdmin()
  if (!admin) return NextResponse.json({ error: 'Não autorizado' }, { status: 403 })

  const body = (await req.json()) as { provaId?: string; numeroEtapa?: number | string }
  const { provaId, numeroEtapa } = body
  if (!provaId || numeroEtapa === undefined || numeroEtapa === null) {
    return NextResponse.json({ error: 'provaId e numeroEtapa são obrigatórios' }, { status: 400 })
  }
